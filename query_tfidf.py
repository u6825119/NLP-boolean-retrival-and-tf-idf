...
def get_doc_to_norm(index, doc_freq, num_docs):
    ...
    for term in index.keys():
        for doc_id, doc_tf in index[term]: 
            doc_norm[doc_id] += helperTFIDF(doc_tf, doc_freq[term], num_docs ) **2 
...
def run_query(query_token_counts, index, doc_freq, doc_norm, num_docs):
    ...
    for query_term, query_tf in query_token_counts: 
        ...
        if query_term not in index:
            continue
        query_norm += helperTFIDF(query_tf, doc_freq[query_term], num_docs)**2
    ...
    for query_term, query_tf in query_token_counts: 
        ...
        for doc_id, doc_tf in index[query_term]:   
            doc_to_score[doc_id] += helperTFIDF(query_tf, doc_freq[query_term], num_docs) * helperTFIDF(doc_tf, doc_freq[query_term], num_docs) / (doc_norm[doc_id] * query_norm)
            #                       
    sorted_docs = sorted(doc_to_score.items(), key=lambda x:-x[1])
    return sorted_docs

def helperTFIDF(tf, doc_freq, num_docs): 
    '''Calculates the tf-idf value
    Args:
        tf (int): term frequency
        doc_freq (int): document frequency
        num_docs (int) : number of documents
        '''
    tf_idf = tf* np.log(num_docs/(1+doc_freq))
    return tf_idf
